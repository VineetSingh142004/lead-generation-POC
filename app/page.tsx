...
const services = [
  { id: "01", name: "Home / Garage Epoxy", copy: "A clean, durable finish for the space that starts and ends your day.", image: "https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=1400&q=88" },
  { id: "02", name: "Commercial / Factory Epoxy", copy: "An intentional surface for high-use spaces and serious work.", image: "https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?auto=format&fit=crop&w=1400&q=88" },
  { id: "03", name: "Other Epoxy Flooring", copy: "Bring us the brief. We will help find the right path forward.", image: "https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?auto=format&fit=crop&w=1400&q=88" },
];
...
export default function Home() {
  ...
  return <main>
    ...
    <section id="services" className="services"><div className="shell"><div className="section-head"><p className="eyebrow">SELECT YOUR PROJECT</p><h2>Built around<br/>the way you use space.</h2><p>Choose your starting point. We’ll take it from there.</p></div><div className="service-list">{services.map((item) => <article key={item.id} className="service-card"><div className="service-image" style={{ backgroundImage: `url(${item.image})` }}/><div className="service-info"><span className="service-index">{item.id}</span><h3>{item.name}</h3><p>{item.copy}</p><button onClick={() => chooseService(item.name)}>Get a quote <span>↗</span></button></div></article>)}</div></div></section>
