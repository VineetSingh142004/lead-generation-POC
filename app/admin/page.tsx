...
export type Lead = { id: string; created_at: string; name: string; phone: string; address: string; service: string; status: string };
...
export default async function AdminPage() {
  ...
  if (!supabase) connectionError = "Connect Supabase to view live leads.";
    ...
    const { data, error } = await supabase.from("leads").select("id, created_at, name, phone, address, service, status").order("created_at", { ascending: false });
